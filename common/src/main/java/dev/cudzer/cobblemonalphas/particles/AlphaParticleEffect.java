    ...
    public static void tick(MinecraftServer server){
        ...
        server.getAllLevels().forEach(level -> {
            level.getAllEntities().forEach( entity -> {
                ...
                ServerPlayer lookingAtAlpha = players.stream().filter(player -> pokemonEntity.getPokemon().isWild() && player.hasLineOfSight(pokemonEntity)).findFirst().orElse(null);
                if(!inRange){
                    return;
                }
                else {
                    if(pokemonEntity.getPokemon().isWild() && !pokemonEntity.isBattling() && !pokemonEntity.isBusy() && lookingAtAlpha != null){
